...
export const SubjectCard = ({ subject, user_id, variant }: SubjectCardProps) => {
  ...
  useEffect(() => {                
    ...
    const handleJoin = useCallback(() => {
      ...
      router.push(`${basePath}/my-classes/completed/${variant === "student" ? subject.id : subject.teacher_user_id}`);
            ...
            {variant === "teacher" ? (
                <>
                  <Button
                    variant="outline" 
                    className="flex-1"
                    onClick={() => router.push(`/${variant}/my-classes/completed/class-details/${subject.id}`)}>
                    Details
                  </Button> 
                  <Button
                    variant="outline"
                    className="flex-1"
                    onClick={() => router.push(`/${variant}/my-classes/completed/view-students/${subject.id}`)}>
                    View Students
                  </Button>
                </>                
            ) : (
